...
from rx.models import FileIndex, IndexType
...
class TestSaveLoadIndex:
    ...
    def test_save_and_load_index(self, temp_text_file):
        ...
        index_path = get_index_path(temp_text_file)
        assert save_index(file_index, index_path)
...
class TestCreateIndexFile:
    ...
    def test_create_index_file_includes_analysis(self, temp_text_file):
        ...
        assert analysis.line_count is not None
        assert analysis.line_length_max is not None
        assert analysis.line_length_avg is not None
        assert analysis.line_length_median is not None
        assert analysis.line_length_p95 is not None
        assert analysis.line_length_p99 is not None
        assert analysis.line_ending is not None
...
class TestOffsetLineMapping:
    ...
    def test_large_file_with_index_performance(self):
        ...
        try:
            ...
            file_index = create_index_file(temp_path, force=True)
            ...
            start = time.time()
            offset_90k = calculate_exact_offset_for_line(temp_path, 90_000, file_index)
            ...
            start = time.time()
            line_num = calculate_exact_line_for_offset(temp_path, offset_90k, file_index)
            duration = time.time() - start
    ...
    def test_many_short_lines_with_index(self):
        ...
        try:
            ...
            start = time.time()
            offset_450k = calculate_exact_offset_for_line(temp_path, 450_000, file_index)
            duration = time.time() - start
    ...
    def test_last_line_after_last_checkpoint(self):
        ...
        try:
            ...
            start = time.time()
            offset = calculate_exact_offset_for_line(temp_path, target_line, file_index)
            duration = time.time() - start
    ...
    def test_calculate_offset_without_passing_index_data(self):
        ...
        try:
            ...
            assert offset != -1, 'BUG: Got -1, function failed to load index from disk'
            assert offset > 0
...
class TestCalculateLineInfoForOffsets:
    ...
    def test_single_offset(self, temp_text_file):
        ...
        assert info.line_start_offset == 0
        assert info.line_end_offset > 0  # Should be the end of line 1

    def test_multiple_offsets_same_line(self, temp_text_file):
        """Test multiple offsets within the same line."""
        from rx.index import calculate_line_info_for_offsets

        # Both offsets 0 and 5 are in line 1
        result = calculate_line_info_for_offsets(temp_text_file, [0, 5])

        assert len(result) == 2
        assert result[0].line_number == 1
        assert result[5].line_number == 1
        # Both should have the same line start/end offsets
        assert result[0].line_start_offset == result[5].line_start_offset
        assert result[0].line_end_offset == result[5].line_end_offset

    def test_multiple_offsets_different_lines(self, temp_text_file):
        """Test multiple offsets across different lines."""
        from rx.index import calculate_line_info_for_offsets

        # Read file to get line boundaries
        with open(temp_text_file, 'rb') as f:
            lines = f.readlines()

        line1_end = len(lines[0])
        line2_start = line1_end
        line2_end = line2_start + len(lines[1])

        # Offset 0 is in line 1, offset at line2_start is in line 2
        result = calculate_line_info_for_offsets(temp_text_file, [0, line2_start])

        assert result[0].line_number == 1
        assert result[line2_start].line_number == 2
        assert result[0].line_start_offset == 0
        assert result[line2_start].line_start_offset == line2_start
    ...
    def test_offsets_processed_efficiently(self, temp_text_file):
        """Test that multiple offsets are processed in a single pass."""
        from rx.index import calculate_line_info_for_offsets

        # Get all line boundaries
        with open(temp_text_file, 'rb') as f:
            content = f.read()

        # Create offsets at start of each line
        offsets = [0]
        for i, char in enumerate(content):
            if char == ord('\n') and i + 1 < len(content):
                offsets.append(i + 1)

        # All offsets should be resolved
        result = calculate_line_info_for_offsets(temp_text_file, offsets)

        for i, offset in enumerate(offsets):
            assert offset in result
            assert result[offset].line_number == i + 1
            assert result[offset].line_start_offset == offset

    def test_line_end_offset_correct(self, temp_text_file):
        """Test that line_end_offset includes the newline character."""
        from rx.index import calculate_line_info_for_offsets

        result = calculate_line_info_for_offsets(temp_text_file, [0])

        info = result[0]
        # Read the actual first line to verify
        with open(temp_text_file, 'rb') as f:
            first_line = f.readline()

        assert info.line_end_offset == len(first_line)

    def test_with_index_data(self, temp_large_file):
        """Test that function works with pre-loaded index data."""
        from rx.index import (
        ...
        try:
            # Create index
            create_index_file(temp_large_file)
            index_data = load_index(get_index_path(temp_large_file))

            # Test with offsets in different parts of file
            offsets = [0, 500, 1000, 5000]
            result = calculate_line_info_for_offsets(temp_large_file, offsets, index_data)

            # All offsets should be resolved
            for offset in offsets:
                assert offset in result
                assert result[offset].line_number >= 1
                assert result[offset].line_start_offset >= 0
                assert result[offset].line_end_offset > result[offset].line_start_offset
        finally:
            delete_index(temp_large_file)

    def test_unsorted_offsets(self, temp_text_file):
        """Test that unsorted offsets are handled correctly."""
        from rx.index import calculate_line_info_for_offsets
        ...
        # Provide offsets in reverse order
        result = calculate_line_info_for_offsets(temp_text_file, [line2_start, 0])
